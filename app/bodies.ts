// The body catalog: physical data plus the JPL elements the orbital math runs on.
// Kept free of Three.js and DOM so tests/orbits.test.mjs can check the real elements
// rather than a copy of them that would quietly drift out of sync.

import type { KeplerianElements, SmallBodyElements } from "./orbits";

export type SmallBodyCategory = "off" | "all" | "comet" | "asteroid" | "outer";
export type BodyName = "Sun" | "Mercury" | "Venus" | "Earth" | "Mars" | "Asteroid Belt" | "Ceres" | "Jupiter" | "Saturn" | "Uranus" | "Neptune" | "Pluto" | "Haumea" | "Makemake" | "Eris" | "Kuiper Belt" | "Moon" | "Phobos" | "Deimos" | "Io" | "Europa" | "Ganymede" | "Callisto" | "Mimas" | "Enceladus" | "Tethys" | "Dione" | "Rhea" | "Titan" | "Iapetus" | "Miranda" | "Ariel" | "Umbriel" | "Titania" | "Oberon" | "Triton" | "Halley" | "Encke" | "67P" | "Hale–Bopp" | "Swift–Tuttle" | "NEOWISE" | "Vesta" | "Pallas" | "Psyche" | "Bennu" | "Chariklo" | "Sedna" | "Arrokoth";
export type Planet = {
  name: BodyName;
  kind: string;
  color: string;
...
export const SMALL_BODIES:Planet[]=[
...
export const NARRATION: Partial<Record<BodyName | "Milky Way" | "Local Group", string[]>> = {
  Sun: [
    ...
    "The Sun is so heavy that everything else in the whole solar system, all the planets put together, is just a tiny crumb beside it.",
    "Sunlight looks white, but it is really every colour mixed together. The sky pulls the blue out of it, and that is why the sky is blue.",
    "Deep inside the Sun it is so crowded that light takes thousands of years just to fight its way out to the surface.",
  ],
  Mercury: [
    ...
    "Mercury has no moons and no rings. It hurries around the Sun all on its own.",
    "Even though Mercury is closest to the Sun, it has ice. Deep in craters near its poles the sunlight never reaches, so it stays frozen.",
    "Mercury is slowly shrinking. As its middle cools, the whole planet wrinkles up, a little like an old apple.",
    "A sunrise on Mercury is worth the wait. Because it turns so slowly, you would stand there for months before the Sun came up.",
  ],
  Venus: [
    ...
    "After the Sun and the Moon, Venus is the brightest thing in our sky. People call it the evening star, though it is really a planet.",
    "Stand on Venus and the sky would not be blue. It would glow a dull, heavy orange, all day and all night.",
    "Long ago Venus may have had oceans, like Earth. They all boiled away, and now it is far too hot for even a drop of water.",
  ],
  Earth: [
    ...
    "Earth is not a perfect ball. It spins so fast that it bulges out a little around the middle.",
    "Deep inside Earth is a ball of iron nearly as hot as the surface of the Sun. Its slow churning is what makes our magnetic field.",
    "Earth's magnetic field wraps right around us like an invisible shield, turning away the wind that streams off the Sun.",
  ],
  Mars: [
    ...
    "A day on Mars is almost exactly as long as a day on Earth. Your mornings and bedtimes would feel about right there.",
    "The daytime sky on Mars is not blue like ours. It is a dusty butterscotch colour, and then at sunset it turns blue.",
    "Mars has ice caps at the top and bottom, just like Earth, made of frozen water and frozen air.",
  ],
  Jupiter: [
    ...
    "Jupiter is a giant ball of gas, mostly the same stuff the Sun is made of. Had it grown much bigger, it might have become a second sun.",
    "Jupiter has rings too, like Saturn, but they are made of fine dust and far too dim to see without a spacecraft.",
    "Jupiter's pull is so strong it can grab a passing comet and fling it right out of the solar system.",
  ],
  Saturn: [
    ...
    "Saturn is not the only planet with rings. Jupiter, Uranus and Neptune have them too. Saturn's are just by far the grandest.",
    "Saturn's rings are so wide that dozens of Earths could be lined up across them.",
    "Saturn has more moons than any other planet, a whole crowd of them, big and small.",
  ],
  Uranus: [
    ...
    "Uranus is the only planet named after a Greek god. All the others are named after Roman ones.",
    "Uranus looks almost perfectly smooth and plain, a calm blue-green ball that gives away none of its secrets.",
    "Only one spacecraft has ever visited Uranus. It flew past long ago, and nothing has been back since.",
    "Uranus is made mostly of icy water and other frozen stuff, which is why we call it an ice giant.",
  ],
  Neptune: [
    ...
    "Neptune is so far away you cannot see it with just your eyes. You need a telescope to find it at all.",
    "Neptune sometimes grows enormous dark storms that swirl for a few years and then quietly fade away.",
    "Only one spacecraft has ever visited Neptune, and it was on its way out of the solar system for good.",
    "Neptune is a deep, rich blue, even bluer than Uranus, and nobody is completely sure why.",
  ],
  Pluto: [
    ...
    "Pluto is smaller than our own Moon. It really is a tiny world, all alone out in the dark.",
    "Pluto's path is tilted and stretched, and for a little while each long trip around, it slips closer to the Sun than Neptune.",
    "Pluto has mountains made of water ice, frozen so hard in the terrible cold that they stand up like rock.",
  ],
  Ceres: [
    ...
    "Ceres is round, which is rare for something so small. Its own gravity pulled it into a ball, the way a planet is.",
    "Ceres was the very first thing ever found in the asteroid belt, so long ago that people first thought it was a new planet.",
    "A spacecraft called Dawn flew all the way to Ceres and circled it for years, mapping every crater.",
    "The brightest spot on Ceres sits in a crater called Occator, glowing like a little lamp in all the grey.",
    "A day on Ceres flies by. It spins all the way round in only about nine hours.",
  ],
  Haumea: [
    ...
    "Haumea has two little moons, named after the daughters of the goddess Haumea itself is named for.",
    "Haumea is covered in bright, clean water ice, so it shines more than most things out that far.",
    "Something once crashed into Haumea and knocked pieces off. Its two moons and its ring may be the leftovers.",
    "Haumea is one of the fastest spinning big things we know, which is what pulled it into its stretched egg shape.",
    "Out where Haumea is, the Sun is just a very bright star, far too small and cold to warm anything.",
  ],
  Makemake: [
    ...
    "Makemake is named after a god from Easter Island, the faraway place where people carved the giant stone heads.",
    "Makemake is one of the biggest things out in the Kuiper belt, though it is still a good deal smaller than Pluto.",
    "Makemake is so far from the Sun that it is one of the coldest places anyone has ever measured.",
    "For years Makemake seemed to have no moon at all, until a telescope finally caught a tiny dark one beside it.",
    "Makemake is coated in frozen methane, and it reflects sunlight almost like fresh snow.",
  ],
  Eris: [
    ...
    "Eris is named after the Greek goddess of trouble and quarrels, because finding it started a great quarrel about what a planet even is.",
    "Eris has a small moon called Dysnomia, circling it far out in the cold and the dark.",
    "Eris is a little heavier than Pluto, even though the two are almost exactly the same size.",
    "Eris is so shiny it is almost a mirror. Its whole surface may be coated in frozen air, fresh and bright.",
    "A year on Eris lasts about five and a half centuries. The last time it sat where it sits now, people still crossed the seas in wooden sailing ships.",
  ],
