...
import { createUniverseGalaxies, createUniverseVolume } from "./universeVolume";
...
function proceduralCmbTexture(){
  // The real WMAP boundary (loaded below) is the normal path; this only covers that fetch
  // failing. Deterministic and clearly a placeholder, not implying it is real observational data.
...
export default function Universe({focusId}:{focusId?:string}){
  ...
  useEffect(()=>{
    ...
    // The bake (scripts/bake-universe-cmb.mjs) already recolors WMAP's real temperature data into
    // the volume's own cool/warm-gold duotone and biases it toward a near-background baseline, so
    // an average patch of sky is already dim — this opacity is headroom for the true bright
    // ripples to read as light, not a second layer of dimming on top of an already-loud texture.
    const cmbFallback=proceduralCmbTexture(),cmbMaterial=new THREE.MeshBasicMaterial({map:cmbFallback,side:THREE.BackSide,depthWrite:false,toneMapped:false,opacity:.58,transparent:true}),cmb=new THREE.Mesh(new THREE.SphereGeometry(172,64,32),cmbMaterial);cmb.rotation.y=.7;scene.add(cmb);lookbackRings(scene);
    ...
    loadUniverseField().then(field=>{if(disposed)return;volume=createUniverseVolume(field);scene.add(volume);scene.add(createUniverseGalaxies(field));setReady(true);}).catch(error=>{console.warn("[universe-field] unable to load volume",error);if(!disposed){mount!.classList.add("no-webgl");setReady(true);}});
