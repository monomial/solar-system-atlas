...
export default function Universe({focusId}:{focusId?:string}){
  ...
  useEffect(()=>{
    ...
    const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.dampingFactor=.045;controls.enablePan=false;controls.minDistance=8;controls.maxDistance=148;
    const cmb=new THREE.Mesh(new THREE.SphereGeometry(172,64,32),new THREE.MeshBasicMaterial({map:proceduralCmbTexture(),side:THREE.BackSide,depthWrite:false,toneMapped:false,opacity:.48,transparent:true}));cmb.rotation.y=.7;scene.add(cmb);
    const labels:DeepLabel[]=[],markers:DeepMarker[]=[],targets:THREE.Object3D[]=[],positions=new Map<string,THREE.Vector3>();let activeId="local-group",fly:{start:number;from:THREE.Vector3;to:THREE.Vector3;targetFrom:THREE.Vector3;targetTo:THREE.Vector3}|null=null,disposed=false,volume:ReturnType<typeof createUniverseVolume>|null=null;
    ...
    // Vantages sit well OUTSIDE the fog around an anchor: diving to arm's length inside a volume
    // this dense reads as overexposed murk, not structure. Phase 2's authored fly-through will
    // earn the inside view; the click-to-focus flight should always keep the web legible.
    function focus(id:string){const target=positions.get(id);if(!target)return;activeId=id;setSelected(id);const radial=target.length(),offset=radial<1?new THREE.Vector3(0,34,52):target.clone().normalize().multiplyScalar(36).add(new THREE.Vector3(0,16,18));fly={start:performance.now(),from:camera.position.clone(),to:target.clone().add(offset),targetFrom:controls.target.clone(),targetTo:target.clone()};}
    function changeView(next:ViewMode){setViewMode(next);const to=next==="top"?new THREE.Vector3(0,142,.01):next==="edge"?new THREE.Vector3(0,3,142):new THREE.Vector3(0,76,112);fly={start:performance.now(),from:camera.position.clone(),to,targetFrom:controls.target.clone(),targetTo:new THREE.Vector3()};}
    ...
    let frame=0;function animate(now:number){frame=requestAnimationFrame(animate);controls.update();cmb.rotation.y+=.000015;if(fly){const t=Math.min(1,(now-fly.start)/1200),ease=t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;camera.position.lerpVectors(fly.from,fly.to,ease);controls.target.lerpVectors(fly.targetFrom,fly.targetTo,ease);if(t>=1)fly=null;}volume?.updateCamera(camera);overlays.update(now);renderer.render(scene,camera);}animate(performance.now());
    function resize(){const width=mount!.clientWidth||1,height=mount!.clientHeight||1;camera.aspect=width/height;camera.updateProjectionMatrix();renderer.setSize(width,height);}const observer=new ResizeObserver(resize);observer.observe(mount);
    return()=>{disposed=true;cancelAnimationFrame(frame);observer.disconnect();overlays.dispose();controls.dispose();scene.traverse(object=>{(object as THREE.Mesh).geometry?.dispose();const material=(object as THREE.Mesh).material;for(const entry of Array.isArray(material)?material:material?[material]:[]){for(const value of Object.values(entry))if(value instanceof THREE.Texture)value.dispose();entry.dispose();}});scene.clear();renderer.dispose();mount.replaceChildren();apiRef.current=null;};
  },[]);
