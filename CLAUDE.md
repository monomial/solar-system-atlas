...
- `npm test` — builds, then runs `tests/` (export integrity + the orbital math).
- `npm run lint`
...
**Ambient mode** (`app/ambient.ts`, `app/useAmbient.ts`) is a lean-back, narrated auto-tour — a third mode beside Explore and Guided Tour, and the web sibling of the tvOS app. It flies outward Sun-to-Eris, speaking one fact per world. It shares everything with the scene: the same math, the same catalog, the same `NARRATION` lines in `bodies.ts`. Three things are worth knowing. (1) The camera is driven through two methods added to `apiRef` — `flyTo(name, onArrive)` and `setAmbient(on)` — so the mount-once scene effect is never rebuilt; the tour orchestration lives entirely in React and calls those. (2) Audio needs a user gesture (browser autoplay policy), which is why there is a "Begin" gate; the Web Audio drone (`createDrone`, ported from the tvOS `AmbientAudio.swift`) and the narration clips both start from that click. (3) The 73 narration clips in `public/narration/` are generated by Kokoro (`npm run narration`) but **committed**, because GitHub Pages cannot run the renderer at deploy time — same reasoning as any built asset that the CI environment cannot reproduce.
